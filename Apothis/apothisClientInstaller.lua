...
local function install()
    ...
    if not fs.exists(".addresses.txt") then
        ...
        local serverName = apothisServers[0].name:sub(1, 16) -- Strip to 16 Symbols
        local file = fs.open(".addresses.txt")
        file.write(server.address)
        file.close()

        serverName = arcanumServers[0].name:sub(1, 16) -- Strip to 16 Symbols
        file = fs.open(".address.txt")
        local addresses = {
            ["apothis"] = file.readAll()
            ["arcanum"] = server.address
        }
